...
const {hashString, createRandomToken, stripePayment} = require('./helpers');
    ...
    if (cartId) {
        ...
        read('cart', cartId, (err, cartData) => {
            if (!err && cartData) {
                ...
                const token = typeof(data.headers.token) == 'string' && data.headers.token.trim().length == authTokenLength ? data.headers.token.trim() : false;
                handlers._tokens.verifyToken(token, cartData.userEmail, tokenIsValid => {
                    if (tokenIsValid) {
...
//Required --> Auth token, Cart Id, payment method
//get the users address, and total price from the cartData
//once the order is posted, remove the order from the cart and remove the ID from the user's object
//STEPS:
//pass it to stripe
//save the order in to the orders DB, 
...
handlers._orders.post = (data, callback) => {
    ...
    const ccNumber = typeof(data.payload.ccNumber) == 'string' && data.payload.ccNumber.trim().length > 0 ? data.payload.ccNumber.trim() : false;
    if (cartId) {
        read('cart', cartId, (err, cartData) => {
            if (!err && cartData) {
                //verify token
                ...
                handlers._tokens.verifyToken(token, cartData.userEmail, tokenIsValid => {
                    if (tokenIsValid) {
                        ...
                        stripePayment('usd', total, `Charge for order at Al Pacino\'s Pizza. order ID: ${orderId}`, 'tok_visa', err => {
                            if (!err) {
                                callback(200, {msg: 'seems the order worked'});
                                //save the cart info to the orders file along with a timestamp of the order and the order ID
                                //delete the cart info from the DB
                                //remove the cart ID from the user
                            } else {
                                callback(500, {error: 'Error processing order.'});
                            }
                        });
                    } else {
                        callback(403, {error: 'Token is missing or invalid.'});
