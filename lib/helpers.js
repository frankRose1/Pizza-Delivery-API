...
/**
 * Used to parse the payload from the client
 * trying to parse anything other than a string will throw an error
 */
...
helpers.stripePayment = (currency, amount, description, source, callback) => {
  ...
  description =
    typeof description == 'string' && description.trim().length > 0
      ...
      : false;
  ccToken =
    typeof ccToken == 'string' && ccToken.trim().length > 0 ? ccToken : false;
...
helpers.mailgunEmail = (from, to, subject, text, callback) => {
  //check the inputs
  from =
    typeof from == 'string' && from.trim().length > 0 ? from.trim() : false;
  ...
  if (from && to && subject && text) {
    ...
    callback(
      'Required fields for sending email via mailgaun are missing or invalid'
    );
  }
};
