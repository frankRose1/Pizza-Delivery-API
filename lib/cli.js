    ...
    if (!err && userEmails) {
      userEmails.forEach(email => {
        ...
        read('users', email, (err, userData) => {
          if (!err && userData) {
            cli.verticalSpace();
            const line = `Name: ${userData.firstName} ${userData.lastName}, Email: ${userData.email}`;
            console.log(line);
            cli.verticalSpace();
          }
        });
    ...
    if (!err && cartIds) {
      ...
      cartIds.forEach(id => {
        read('cart', id, (err, cartData) => {
          if (!err && cartData) {
            cli.verticalSpace();
            console.log(`Cart ID: ${cartData.id}, Cart Owner: ${cartData.userEmail}`);
            cli.verticalSpace();
          }
        });
      });
...
cli.responders.moreCartInfo = str => {
  const arr = str.split('--');
  const cartId = typeof(arr[1]) == 'string' && arr[1].trim().length > 0 ? arr[1].trim() : false;
  if (cartId) {
    ...
    if (!err && orderIds) {
      orderIds.forEach(id => {
        read('orders', id, (err, orderData) => {
          if (!err && orderData) {
            cli.verticalSpace();
            console.log(`Order ID: ${orderData.id}, Order Owner: ${orderData.userEmail}`);
            cli.verticalSpace();
          }
        });
      });
